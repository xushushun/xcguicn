...
func NewPane(pName string, nWidth int, nHeight int, hFrameWnd int) *Pane {
	p := &Pane{}
	p.W置句柄(xc.XPane_Create(pName, nWidth, nHeight, hFrameWnd))
	return p
}
...
func (p *Pane) SetView(hView int) int {
	return xc.XPane_SetView(p.W句柄, hView)
}
...
func (p *Pane) SetTitle(pTitle string) int {
	return xc.XPane_SetTitle(p.W句柄, pTitle)
}

// 窗格_取标题, 获取标题文本.
func (p *Pane) GetTitle() string {
	return xc.XPane_GetTitle(p.W句柄)
}
...
func (p *Pane) SetCaptionHeight(nHeight int) int {
	return xc.XPane_SetCaptionHeight(p.W句柄, nHeight)
}

// 窗格_取标题栏高度, 获取标题栏高度.
func (p *Pane) GetCaptionHeight() int {
	return xc.XPane_GetCaptionHeight(p.W句柄)
}

// 窗格_判断显示, 判断窗格是否显示.
func (p *Pane) IsShowPane() bool {
	return xc.XPane_IsShowPane(p.W句柄)
}
...
func (p *Pane) SetSize(nWidth int, nHeight int) int {
	return xc.XPane_SetSize(p.W句柄, nWidth, nHeight)
}

// 窗格_取状态, 获取窗格停靠状态, 返回: Pane_State_.
func (p *Pane) GetState() xcc.Pane_State_ {
	return xc.XPane_GetState(p.W句柄)
}
...
func (p *Pane) GetViewRect(pRect *xc.RECT) int {
	return xc.XPane_GetViewRect(p.W句柄, pRect)
}
...
func (p *Pane) HidePane(bGroupActivate bool) int {
	return xc.XPane_HidePane(p.W句柄, bGroupActivate)
}
...
func (p *Pane) ShowPane(bGroupActivate bool) int {
	return xc.XPane_ShowPane(p.W句柄, bGroupActivate)
}

// 窗格_停靠, 窗格停靠到码头.
func (p *Pane) DockPane() int {
	return xc.XPane_DockPane(p.W句柄)
}

// 窗格_锁定, 锁定窗格.
func (p *Pane) LockPane() int {
	return xc.XPane_LockPane(p.W句柄)
}

// 窗格_浮动.
func (p *Pane) FloatPane() int {
	return xc.XPane_FloatPane(p.W句柄)
}
